    ...
    <div class="container">
        ...
        <div class="grid2col">
            ...
            <div class="panel">
                ...
                <div id="vlessFields"><div class="row"><div><label>UUID</label><input id="vlessUuid" class="ltr"></div><button class="btn btn-outline" id="genVlessUuid">🎲</button></div><div><label>Flow</label><select id="vlessFlow" class="ltr"><option value="">None</option><option>xtls-rprx-vision</option><option>xtls-rprx-vision-udp443</option><option>xtls-rprx-origin</option><option>xtls-rprx-direct</option></select></div><div><label>Encryption</label><input id="vlessEncryption" class="ltr" value="none"></div></div>
                <div id="vmessFields" style="display:none;"><div class="row"><div><label>UUID</label><input id="vmessUuid" class="ltr"></div><button class="btn btn-outline" id="genVmessUuid">🎲</button></div><div><label>Security</label><select id="vmessSecurity" class="ltr"><option>auto</option><option>aes-128-gcm</option><option>chacha20-poly1305</option><option>none</option><option>zero</option></select></div><div><label>Alter ID</label><input type="number" id="vmessAlterId" class="ltr" value="0"></div></div>
        ...
        <div class="grid2col">
            <div class="panel"><h2>🌍 IP / Domain List</h2><textarea id="ipList" class="ltr" placeholder="1.1.1.1&#10;my.server.com&#10;..."></textarea><div style="margin-top:12px;"><button class="btn btn-primary" id="generateBtn">🚀 Generate Configs</button><button class="btn btn-outline" id="clearIpBtn">🗑️ Clear</button></div></div>
...
(function() {
    ...
    // Parsing functions (exactly as final version)
    ...
    function parseVMess(link) {
        ...
        return { protocol:'vmess', address: json.add, port: json.port, uuid: json.id, aid: json.aid || 0, security: json.scy || 'auto', net: json.net || 'tcp', tls: json.tls==='tls'?'tls':'none', sni: json.sni || json.host || '', fp: json.fp || '', alpn: json.alpn || '', path: json.path || '', host: json.host || '', serviceName: json.path || '', remark: json.ps || '' };
    }
    function parseVLess(link) {
        const parsed = parseCustomScheme(link, 'vless');
        const p = parsed.params;
        return { protocol:'vless', address: parsed.host, port: parsed.port, uuid: parsed.userinfo, remark: parsed.fragment, flow: p.flow||'', enc: p.encryption||'none', security: p.security||'none', sni: p.sni||'', fp: p.fp||'', alpn: p.alpn||'', pbk: p.pbk||'', sid: p.sid||'', spx: p.spx||'/', net: p.type||'tcp', path: p.path||'/', host: p.host||'', serviceName: p.serviceName||'', kcpSeed: p.seed||'', kcpHeader: p.headerType||'', kcpCong: p.congestion||'', quicSec: p.quicSecurity||'', quicKey: p.key||'', quicHeader: p.quicHeaderType||'', h2Host: p.host||'', h2Path: p.path||'', httpupgradeHost: p.host||'', httpupgradePath: p.path||'', splithttpHost: p.host||'', splithttpPath: p.path||'', xhttpHost: p.host||'', xhttpPath: p.path||'', xhttpMode: p.mode||'' };
    }
    function parseTrojan(link) {
        const parsed = parseCustomScheme(link, 'trojan');
        const p = parsed.params;
        return { protocol:'trojan', address: parsed.host, port: parsed.port, password: parsed.userinfo, remark: parsed.fragment, security: p.security||'tls', sni: p.sni||'', fp: p.fp||'', alpn: p.alpn||'', net: p.type||'tcp', path: p.path||'/', host: p.host||'', serviceName: p.serviceName||'', kcpSeed: p.seed||'', kcpHeader: p.headerType||'', kcpCong: p.congestion||'', quicSec: p.quicSecurity||'', quicKey: p.key||'', quicHeader: p.quicHeaderType||'', h2Host: p.host||'', h2Path: p.path||'', httpupgradeHost: p.host||'', httpupgradePath: p.path||'', splithttpHost: p.host||'', splithttpPath: p.path||'', xhttpHost: p.host||'', xhttpPath: p.path||'', xhttpMode: p.mode||'' };
    }
    function parseSS(link) {
        ...
        if(atIdx !== -1) {
            ...
            const hostPart = main.slice(atIdx+1).split(':');
            addr = hostPart[0]; port = hostPart[1] || '8388';
        } else {
            ...
            const hp = host.split(':');
            addr = hp[0]; port = hp[1] || '8388';
        }
        return { protocol:'ss', address: addr, port: parseInt(port), method, password, remark, security:'none', net:'tcp' };
    }
    ...
    function importLinks() {
        ...
        try {
            ...
            for(const line of lines) {
                const l = line.trim();
                let addr = '';
                if(l.startsWith('vmess://')) addr = JSON.parse(safeAtob(l.slice(8))).add;
                    ...
                    if(at !== -1) addr = main.slice(at+1).split(':')[0];
                    else { const dec = safeAtob(main); addr = dec.split('@')[1]?.split(':')[0] || ''; }
                }
    ...
    // Config generation (identical to final version)
    ...
    function buildLink(server, index) {
        ...
        switch (currentProtocol) {
            ...
            case 'vless': {
                ...
                return \`vless://\${val('vlessUuid')}@\${server}:\${port}\${queryStr}#\${name}\`;
            }
            case 'trojan': {
                const queryStr = params.length ? '?' + params.join('&') : '';
                return \`trojan://\${val('trojanPassword')}@\${server}:\${port}\${queryStr}#\${name}\`;
            }
            case 'ss': {
    ...
    // Output rendering (identical to final)
    ...
    async function measureLatency(cfg) {
        ...
        const wsUrl = \`\${scheme}://\${sni || server}:\${port}\${wsPath}\`;
        try {
